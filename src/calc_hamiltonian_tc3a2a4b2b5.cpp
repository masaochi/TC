...
//    three_body_factor_b5 = 0.0;
    ...
    std::vector<std::vector<Eigen::VectorXd> > uG(num_independent_spins);
    for (int ispin1=0; ispin1<num_independent_spins; ispin1++)
    {
        uG[ispin1].resize(2);
        for (int ispin2=0; ispin2<2; ispin2++)
        {
            uG[ispin1][ispin2] = Eigen::VectorXd::Zero(plane_wave_basis.size_FFT_grid());
            for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
            {
                uG[ispin1][ispin2](ipw) = potentials.jastrow.uk(Gvect[ipw], ispin1, ispin2);
            }
        }
    }
    std::vector<std::vector<Eigen::VectorXd> > uk_save;
    std::vector<std::vector<Eigen::VectorXcd> > qsum_temp_3a2(num_independent_spins);
    std::vector<std::vector<Eigen::VectorXcd> > qsum_temp_3a4;
    ...
    std::vector<Eigen::VectorXcd> qsum_temp_3a4_trans(3);
    for (int idim=0; idim<3; idim++)
    {
        qsum_temp_3a4_trans[idim] = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid());
    }
    ...
    std::vector<std::vector<Eigen::VectorXcd> > qsum_temp_3b5;
    ...
    std::vector<Eigen::VectorXcd> qsum_temp_3b5_trans(3);
    for (int idim=0; idim<3; idim++)
    {
        qsum_temp_3b5_trans[idim] = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid());
    }
    ...
    for (int ispinq=0; ispinq<num_independent_spins; ispinq++) // spin of |q1> and |q2>
    {
        ...
        for (int iq2=0; iq2<num_irreducible_kpoints_scf; iq2++)
        {
            ...
            const int nbands2_total = nbands2_old + bloch_states.num_occupied_bands()[ispinq][iq2];
            q2vect = crystal_structure.reciprocal_vectors().transpose() *
                kpoints.kvectors_scf()[iq2][0];

            for (int ispin=0; ispin<num_independent_spins; ispin++)
            {
                qsum_temp_3a2[ispin].resize(nbands2_total);
                for (int ibandq2_shifted=0; ibandq2_shifted<nbands2_total; ibandq2_shifted++) // Note! differs from ibandq2
                {
                    qsum_temp_3a2[ispin][ibandq2_shifted] = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid());
                }
            }
            qsum_temp_3a4.resize(nbands2_total);
            qsum_temp_3b5.resize(nbands2_total);
            for (int ibandq2_shifted=0; ibandq2_shifted<nbands2_total; ibandq2_shifted++) // Note! differs from ibandq2
            {
                qsum_temp_3a4[ibandq2_shifted].resize(3);
                qsum_temp_3b5[ibandq2_shifted].resize(3);
                for (int idim=0; idim<3; idim++)
                {
                    qsum_temp_3a4[ibandq2_shifted][idim] = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid());
                    qsum_temp_3b5[ibandq2_shifted][idim] = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid());
                }
            }

            /*** [3a4,b5] ***/
            uk_save.resize(num_irreducible_kpoints);
            for (int ik=0; ik<num_irreducible_kpoints; ik++)
            {
                ...
                uk_save[ik].resize(kpoints.kvectors_scf()[iq2].size());
                ...
                {
                    uk_save[ik][isymq2] = Eigen::VectorXd::Zero(plane_wave_basis.size_FFT_grid());

                    kqvect = crystal_structure.reciprocal_vectors().transpose() *
                        (kvector_ref - kpoints.kvectors_scf()[iq2][isymq2]);
#ifdef _OPENMP
                    #pragma omp parallel for
#endif
                    for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                    {
                        uk_save[ik][isymq2](ipw) = potentials.jastrow.uk(kqvect + Gvect[ipw], ispinq, ispinq);
                    } // ipw
                } // isymq2
            } // ik
            /*** [3a4,b5] ***/

            // Set (iq1,isymq1) to be the outer loops (Not ibandq2!) (while (iq2, ibandq2) are paired indices),
            // to avoid expensive evaluation of the Jastrow function in a deeply nested loop.
            // Note: Jastrow function depends on wave vectors, not band indices.
            for (int iq1=0; iq1<num_irreducible_kpoints_scf; iq1++)
            {
                for (int isymq1=0; isymq1<kpoints.kvectors_scf()[iq1].size(); isymq1++)
                {
                    ...
                    // Note that a ibandq2 loop is inside the (iq1,isymq1) loops
#ifdef _OPENMP
                    #pragma omp parallel firstprivate(phiq2, chiq2, phiq1, chiq1, phiphi, phij, orbital, orbital2)
                    {
                        ...
                        for (int ibandq2=-nbands2_old; ibandq2<bloch_states.num_occupied_bands()[ispinq][iq2]; ibandq2++)
                        {
                            ...
                            plane_wave_basis_thread.FFT_backward(phiq2, phiq2);
                            ...
                            if (is_bitc)
                            {
                                ...
                                plane_wave_basis_thread.FFT_backward(chiq2, chiq2);
                            }
                            ...
                            for (int ibandq1=-nbands1_old; ibandq1<bloch_states.num_occupied_bands()[ispinq][iq1]; ibandq1++)
                            {
                                ...
                                plane_wave_basis_thread.FFT_backward(phiq1, phiq1);
                                ...
                                if (is_bitc)
                                {
                                    ...
                                    plane_wave_basis_thread.FFT_backward(chiq1, chiq1);
                                }
                                
                                // <*,q1,*| |*,q2,*>
                                phiphi = chiq1_ref.conjugate().array() * phiq2.array();
                                plane_wave_basis_thread.FFT_forward(phiphi, phiphi);
                                
                                if (is_bitc)
                                {
                                    // (<*,*,q2| |*,*,q1>)^* = <*,*,phi_q1| |*,*,chi_q2>
                                    phij = phiq1.conjugate().array() * chiq2_ref.array();
                                    plane_wave_basis_thread.FFT_forward(phij, phij); // = phiphi (for !is_bitc)
                                }
                                ...
                                for (int ispin=0; ispin<2; ispin++)
                                {
                                    ...
                                    for (int idim=0; idim<3; idim++)
                                    {
                                        ...
                                        plane_wave_basis_thread.FFT_backward(orbital, orbital); // idim-component
                                        ...
                                        if (is_bitc)
                                        {
                                            ...
                                            plane_wave_basis_thread.FFT_backward(orbital2, orbital2); // idim-component                                            
#ifdef _OPENMP
                                            #pragma omp critical
#endif
                                            {
                                                // <*,q1,q2| \nabla_1 u_12 \nabla_1 u_13 |*,q2,q1>
                                                qsum_temp_3a2[ispin_ref][ibandq2 + nbands2_old] // ibandq2 + nbands2_old = ibandq2_shifted
                                                    = qsum_temp_3a2[ispin_ref][ibandq2 + nbands2_old].array() - // -: conj(I) for orbital2
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                // <*,q1,q2| \nabla_1 u_12 \nabla_1 u_13 |*,q2,q1>
                                                qsum_temp_3a2[ispin_ref][ibandq2 + nbands2_old]
                                                    = qsum_temp_3a2[ispin_ref][ibandq2 + nbands2_old].array() - // -: conj(I) for orbital2
...
#endif
                                        {
                                            if (ispin==ispinq) // parallel-spin only
                                            {
                                                /*** [3a4(,b2,b5 for TC)] ***/
                                                // <*,q1,*| \nabla_1 u_12 |q1,q2,*>
                                                qsum_temp_3a4[ibandq2 + nbands2_old][idim] 
                                                    = qsum_temp_3a4[ibandq2 + nbands2_old][idim].array() +
                                                ...
                                                if (is_bitc)
                                                {
                                                    ...
                                                    qsum_temp_3b5[ibandq2 + nbands2_old][idim]
                                                        = qsum_temp_3b5[ibandq2 + nbands2_old][idim].array() - // -: conj(I)
            ...
            for (int ibandq2=-nbands2_old; ibandq2<bloch_states.num_occupied_bands()[ispinq][iq2]; ibandq2++)
            {
                ...
                for (int ispin=0; ispin<num_independent_spins; ispin++)
                {
                    MPI_Allreduce(qsum_temp_3a2[ispin][ibandq2 + nbands2_old].data(),
                        ...
                        for (int ispin=0; ispin<2; ispin++)
                        {
                            ...
                            for (int ipw=1; ipw<plane_wave_basis.size_FFT_grid(); ipw++) // no G=0 term! (ipw!=0)
                            {
                                qsum_temp_temp[ispin_ref](ipw) -= 
                                    coefftmp * uG[ispinq][ispin][ipw] * (phiphi(ipw)*phij(0) + phiphi(0)*phij(ipw));
                            } // ipw
                        } // ispin
                    } // ibandq1
                ...
                for (int isymq2=0; isymq2<kpoints.kvectors_scf()[iq2].size(); isymq2++)
                {
                    ...
                    for (int ispin=0; ispin<num_independent_spins; ispin++)
                    {
                        qsum_temp_3a2[ispin][ibandq2 + nbands2_old] = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid());
                    }
                    if (kpoints.is_time_reversal_used_at_k()[iq2][isymq2])
                    {
                        for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                        {
                            ...
                            for (int ispin=0; ispin<num_independent_spins; ispin++)
                            {
                                qsum_temp_3a2[ispin][ibandq2 + nbands2_old](ipw_trans) 
                                    += phase * std::conj(qsum_temp_temp[ispin](ipw));
                            }
                        } // ipw
                    }
                    else // time-reversal symmetry is not used for this k-point
                    {
                        for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                        {
                            ...
                            {
                                qsum_temp_3a2[ispin][ibandq2 + nbands2_old](ipw_trans) 
                                    += phase * qsum_temp_temp[ispin](ipw);
                            }
                        } // ipw
                    } // if (time_reversal)
                    ...
                    for (int ispin=0; ispin<num_independent_spins; ispin++)
                    {
                        V_3a2[ispin] = V_3a2[ispin].array() +
                            filq2 * qsum_temp_3a2[ispin][ibandq2 + nbands2_old].array();
                    }
                } // isymq2
                /*** [3a2] ***/
                ...
                for (int idim=0; idim<3; idim++)
                {
                    MPI_Allreduce(qsum_temp_3a4[ibandq2 + nbands2_old][idim].data(),
                                  qsum_temp_temp[idim].data(),
                                  qsum_temp_temp[idim].size(),
                                  MPI_CXX_DOUBLE_COMPLEX, MPI_SUM, MPI_COMM_WORLD);
                    if (is_bitc)
                    {
                        MPI_Allreduce(qsum_temp_3b5[ibandq2 + nbands2_old][idim].data(),
                ...
                // (k'(iq2,isymq2) = U^+ k(iq2,0))
#ifdef _OPENMP
                #pragma omp parallel firstprivate(qsum_temp_3a4_trans, qsum_temp_3b5_trans, q2symvect, phiq2, chiq2, kvect, kqvect, du, phij, orbital2, phiphi, orbital)
                {
                    PlaneWaveBasis plane_wave_basis_thread;
                    #pragma omp critical // making a plan in FFTW is not thread-safe.
                    {
                    ...
                    for (int isymq2=0; isymq2<kpoints.kvectors_scf()[iq2].size(); isymq2++)
                    {
                        ...
                        if (kpoints.is_time_reversal_used_at_k()[iq2][isymq2])
                        {
                            for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                            {
                                ...
                                for (int idim=0; idim<3; idim++)
                                {
                                    qsum_temp_3a4_trans[idim](ipw_trans) -= phase *
                                    ...
                                    qsum_temp_3b5_trans[idim](ipw_trans) -= phase *
                        ...
                        for (int idim=0; idim<3; idim++)
                        {
                            plane_wave_basis_thread.FFT_backward(qsum_temp_3a4_trans[idim], qsum_temp_3a4_trans[idim]);
                            plane_wave_basis_thread.FFT_backward(qsum_temp_3b5_trans[idim], qsum_temp_3b5_trans[idim]);
                        }
                        /*** [3a4,b2,b5] ***/
                        ...
                        {                        
                            ...
                            for (int idim=0; idim<3; idim++)
                            {
                                qsum_temp_3b2[idim] = qsum_temp_3b2[idim].array() +
                                    filq2 * qsum_temp_3b5_trans[idim].array() * phiq2.array();
                            }
                            /*** [3b2] ***/
                        } // pragma omp critical
                        ...
                        for (int ik=0; ik<num_irreducible_kpoints; ik++)
                        {
                            ...
                            for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                            {
                                for (int idim=0; idim<3; idim++)
                                {
                                    du[0][idim](ipw) = (kqvect(idim) + Gvect[ipw](idim)) * uk_save[ik][isymq2](ipw);
                                }
                            }
                            for (int jband=0; jband<H3phi[ispinq][ik].size(); jband++)
                            {
                                ...
                                for (int jspinor=0; jspinor<num_spinor; jspinor++)
                                {
                                    ...
                                    plane_wave_basis.get_orbital_FFTgrid(ispinq, ik, 0, // isym = 0
                                                                         ...
                                                                         method.calc_mode());
                                    plane_wave_basis_thread.FFT_backward(phij, phij); // -> phij(R)

                                    orbital2 = Eigen::VectorXcd::Zero(plane_wave_basis.size_FFT_grid()); // dummy of H3phi here

                                    /*** [3a4] ***/
                                    // <*,*,q2| |*,*,j>
                                    Eigen::VectorXcd &chiq2_ref_thread = is_bitc ? chiq2 : phiq2; // bra orbital
                                    phiphi = chiq2_ref_thread.conjugate().array() * phij.array();
                                    plane_wave_basis_thread.FFT_forward(phiphi, phiphi);
                                    ...
                                    for (int idim=0; idim<3; idim++)
                                    {
                                        ...
                                        orbital2 = orbital2.array() +
                                            qsum_temp_3a4_trans[idim].array() * orbital.array();
                                    }
                                    /*** [3a4] ***/
                                    ...
                                    {
                                        // <*,q1,q2| \nabla_2 u_23 |*,j,q1>
                                        orbital = qsum_temp_3b5_trans[idim].array() * phij.array();
                                        plane_wave_basis_thread.FFT_forward(orbital, orbital); // idim-component
                                        
                                        // <*,q1,q2| \nabla_2 u_21 \nabla_2 u_23 |*,j,q1>
                                        phiphi = phiphi.array() + orbital.array() * du[0][idim].array();
                                    }
                                    plane_wave_basis_thread.FFT_backward(phiphi, phiphi);
                                    ...
                                    /*** [3a4,b5] ***/
                                    plane_wave_basis_thread.FFT_forward(orbital2, orbital2);
#ifdef _OPENMP
                                    #pragma omp critical
#endif
                                    {
        ...
        for (int ispin=0; ispin<num_independent_spins; ispin++)
        {
            if (num_independent_spins==1)
            {
                for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                {
                    double uk_value = uG[ispin][0](ipw) + uG[ispin][1](ipw); // summation over ispinq
                    for (int idim=0; idim<3; idim++)
                    {
                        du[0][idim](ipw) = Gvect[ipw](idim) * uk_value;
                    }
                }
            }
            else // num_independent_spins==2
            {
                for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                {
                    double uk_value = uG[ispin][ispinq](ipw);
                    for (int idim=0; idim<3; idim++)
                    {
                        du[0][idim](ipw) = Gvect[ipw](idim) * uk_value;
                    }
    ...
    if (potentials.includes_div_correction() && !potentials.jastrow.is_A_zero())
    {
        ...
        {
#ifdef _OPENMP
            #pragma omp parallel firstprivate(kvect, kVaux, q1vect, kqvect, du, phiq2, chiq2, orbital, orbital2, phiq1, chiq1, phiphi, phij)
            {
                PlaneWaveBasis plane_wave_basis_thread;
                #pragma omp critical // making a plan in FFTW is not thread-safe.
                {
                ...
                for (int ik=0; ik<num_irreducible_kpoints; ik++)
                {
                    ...
                    {
                        // To avoid expensive Jastrow evaluation,
                        // we set (iq1,isymq1) loop outside the ibandk2 loop.
                        for (int iq1=0; iq1<num_irreducible_kpoints_scf; iq1++)
                        {
                            for (int isymq1=0; isymq1<kpoints.kvectors_scf()[iq1].size(); isymq1++)
                            {
                                ...
                                for (int ipw=0; ipw<plane_wave_basis.size_FFT_grid(); ipw++)
                                {
                                    ...
                                    du[0][0](ipw) = kVaux.dot(kqvect + Gvect[ipw]) * uk_value; // kVaux * du
                                }
                                
                                // ibandk2 loop is inside the (iq1,isymq2) loop
                                ...
                                for (int ibandk2=-nbands2_old; ibandk2<num_bands_tc[ispin]; ibandk2++)
                                {
                                    ...
                                    plane_wave_basis_thread.FFT_forward(orbital, orbital); // *** [3a4] ***
                                    plane_wave_basis_thread.FFT_forward(orbital2, orbital2); // *** [3b5] ***
