                                     ...
                                     std::ostream *ost)
{
    ...
    for (int ispin=0; ispin<num_independent_spins; ispin++)
    {
        ...
        for (int ik=0; ik<num_irreducible_kpoints; ik++)
        {
            ...
            for (int iband=0; iband<nbands; iband++) // initialize V by phik
            {
                ...
                if (is_bitc) // orthonormalization required
                {
    ...
    for (int iter_inner=0; iter_inner<num_refresh_david_; iter_inner++) // update trial vectors while keeping the Fock operator unchanged (refresh)
    {
                ...
                for (int ispin=0; ispin<num_independent_spins; ispin++)
                {
                    for (int ik=0; ik<num_irreducible_kpoints; ik++)
                    {
                        ...
                        for (int iband=0; iband<nbands; iband++)
                        {
                            ...
                            if (Gram_Schmidt(V[ispin][ik], ndim_total_temporary,
                                             phi[ispin][ik][iband][0])) // true = linearly-independent phi
                            {
        ...
        // To construct subspace Hamiltonian, we need to evaluate remaining matrix elements of HV
        for (int ispin=0; ispin<num_independent_spins; ispin++)
        {
            for (int ik=0; ik<num_irreducible_kpoints; ik++)
            {
            ...
            for (int ispin=0; ispin<num_independent_spins; ispin++)
            {
                for (int ik=0; ik<num_irreducible_kpoints; ik++)
                {
                    ...
                    for (int idim=0; idim<ndim_total; idim++)
                    {
                        ...
                        {
                            H1_subspace[ispin][ik](idim, jdim) = V[ispin][ik][idim].dot(H1V[ispin][ik][jdim]);
                            H2_subspace[ispin][ik](idim, jdim) = V[ispin][ik][idim].dot(H2V[ispin][ik][jdim]);
                            if (uses_3body)
                            {
                                H3_subspace[ispin][ik](idim, jdim) = V[ispin][ik][idim].dot(H3V[ispin][ik][jdim]);
                            }
                        }
                    }
                    ...
                    // A = V*D*V^{-1} where V = diag.eigenvectors()
