...
void io_qe_files::upf::read_upf(FileNames &file_names,
                                ...
                                std::ostream *ost)
{
    if (am_i_mpi_rank0) // open upf files
    {
        *ost << " Read_upf" << std::endl;
        ...
        std::vector<int> atomic_number(num_atomic_species);
        ...
        for (int iatomic_species=0; iatomic_species<num_atomic_species; iatomic_species++)
        {
            *ost << "  Species = " << iatomic_species << std::endl;
            ...
            read_upf_header(atomic_species, ifs, file_names.upf()[iatomic_species],
                            is_upf_ver2, atomic_number[iatomic_species],
        ...
        *ost << " set atomic_number & Z_valence" << std::endl;
        crystal_structure.set_upf(atomic_number, Z_valence); 
        *ost << " set_upf" << std::endl;
        potentials.set_upf(pseudo_rmesh, pseudo_weight_rmesh,
                           pseudo_local, pseudo_beta, pseudo_lbeta, pseudo_dij,
                           crystal_structure, plane_wave_basis);
    }
    if (am_i_mpi_rank0) { *ost << " Bcast upf variables" << std::endl; }
    potentials.bcast_upf(am_i_mpi_rank0);
    crystal_structure.bcast_upf(am_i_mpi_rank0);
    if (am_i_mpi_rank0) { *ost << std::endl; }
}
    
bool io_qe_files::upf::check_upf_ver2(std::ifstream &ifs, std::ostream *ost)
{
    std::string sline;
    std::getline(ifs, sline);
...
// read PP_HEADER
void io_qe_files::upf::read_upf_header(const AtomicSpecies &atomic_species,
                                       ...
                                       std::ostream *ost)
{
    ...
    if (is_upf_ver2) // UPF ver.2
    {
        ...
        *ost << "   Z_valence = " << Z_valence << std::endl;
        ...
        std::string element_name = read_upf_element(pt, "UPF.PP_HEADER.<xmlattr>.element", ost);
        *ost << "   element_name = " << element_name << std::endl;
        atomic_number = atomic_species.get_number(element_name);
        *ost << "   atomic number = " << atomic_number << std::endl;
    ...
    else // UPF ver.1
    {
        ...
        s1 = getline_and_return_first(ifs); // 2nd line: element name
        *ost << "   element_name = " << s1 << std::endl;
        atomic_number = atomic_species.get_number(s1);
        *ost << "   atomic number = " << atomic_number << std::endl;
...
// read PP_MESH
void io_qe_files::upf::read_upf_mesh(std::ifstream &ifs, std::vector<double> &pseudo_rmesh,
...
// read PP_LOCAL
void io_qe_files::upf::read_upf_local(std::ifstream &ifs, std::vector<double> &pseudo_local)
{
    ifs.clear();
    ifs.seekg(0, std::ios_base::beg);
...
// read PP_NONLOCAL    
void io_qe_files::upf::read_upf_nonlocal(std::ifstream &ifs, const bool is_upf_ver2,
...
void io_qe_files::upf::read_upf_nlcc(std::ifstream &ifs, std::vector<double> &pseudo_nlcc)
{
    ifs.clear();
    ifs.seekg(0, std::ios_base::beg);
