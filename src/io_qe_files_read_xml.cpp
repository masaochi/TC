...
std::string read_xml_element(const boost::property_tree::ptree &pt, const std::string &element_name,
                             std::ostream *ost)
{
...
std::string read_xml_element_child(const boost::property_tree::ptree &pt, const std::string &tree_name,
                                   const std::string &element_name, std::ostream *ost)
{
...
void io_qe_files::read_xml(FileNames &file_names,
                           const Method &method,
                           ...
                           std::ostream *ost)
{
    ...
    *ost << " Read QE information from " << qe_xml << std::endl;
    ...
    method.check_consistency_of_calc_mode(read_xml_element(pt, "qes:espresso.input.control_variables.calculation", ost));
    
    // lattice and reciprocal vectors
    dvect_tmp1.resize(3);
    Eigen::Matrix3d lattice_vectors, reciprocal_vectors, reciprocal_vectors_normalized;

    string_to_dvect(read_xml_element(pt, "qes:espresso.output.atomic_structure.cell.a1", ost), dvect_tmp1);
    for (int idim=0; idim<3; idim++) { lattice_vectors(0, idim) = dvect_tmp1[idim]; }
    string_to_dvect(read_xml_element(pt, "qes:espresso.output.atomic_structure.cell.a2", ost), dvect_tmp1);
    for (int idim=0; idim<3; idim++) { lattice_vectors(1, idim) = dvect_tmp1[idim]; }
    string_to_dvect(read_xml_element(pt, "qes:espresso.output.atomic_structure.cell.a3", ost), dvect_tmp1);
    for (int idim=0; idim<3; idim++) { lattice_vectors(2, idim) = dvect_tmp1[idim]; }

    string_to_dvect(read_xml_element(pt, "qes:espresso.output.basis_set.reciprocal_lattice.b1", ost), dvect_tmp1);
    for (int idim=0; idim<3; idim++) { reciprocal_vectors_normalized(0, idim) = dvect_tmp1[idim]; }
    string_to_dvect(read_xml_element(pt, "qes:espresso.output.basis_set.reciprocal_lattice.b2", ost), dvect_tmp1);
    for (int idim=0; idim<3; idim++) { reciprocal_vectors_normalized(1, idim) = dvect_tmp1[idim]; }
    string_to_dvect(read_xml_element(pt, "qes:espresso.output.basis_set.reciprocal_lattice.b3", ost), dvect_tmp1);
    ...
    const double alat = boost::lexical_cast<double>(read_xml_element(pt, "qes:espresso.output.atomic_structure.<xmlattr>.alat", ost));
    reciprocal_vectors = (2*PI/alat)*reciprocal_vectors_normalized;

    // atom informations
    double num_atomic_species = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.atomic_species.<xmlattr>.ntyp", ost));
        ...
        if (child.first=="species")
        {
            *ost << "  Species: " << ispecies << std::endl;
            ...
            name_atomic_species[ispecies] 
                = read_xml_element_child(child.second, "qes:espresso.output.atomic_species.species", "<xmlattr>.name", ost);
            pseudo_file_atomic_species[ispecies] 
                = read_xml_element_child(child.second, "qes:espresso.output.atomic_species.species", "pseudo_file", ost);
            ispecies++;
        }
    }
    ...
    const int num_atoms = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.atomic_structure.<xmlattr>.nat", ost));
    ...
    BOOST_FOREACH (auto child, pt.get_child("qes:espresso.output.atomic_structure.atomic_positions")) 
    {
        if (child.first=="atom")
        {
            *ost << "  Atom: " << iatom << std::endl;
            std::string name 
                = read_xml_element_child(child.second, "qes:espresso.output.atomic_structure.atomic_positions.atom", "<xmlattr>.name", ost);
    ...
    crystal_structure.set(lattice_vectors, reciprocal_vectors, num_atomic_species, index_of_atoms, atomic_position_cartesian, ost);

    // spin    
    spin.set(read_xml_element(pt, "qes:espresso.output.magnetization.noncolin", ost),
             read_xml_element(pt, "qes:espresso.output.magnetization.lsda", ost));
    
    // symmetry operations
    const int num_symmetry_operations = boost::lexical_cast<int>(read_xml_element(pt,"qes:espresso.output.symmetries.nsym",ost));
    ...
    BOOST_FOREACH (auto child, pt.get_child("qes:espresso.output.symmetries")) 
    {
        if (child.first=="symmetry")
        {
            ...
            string_to_dvect(read_xml_element_child(child.second, "qes:espresso.output.symmetries.symmetry",
    ...
    symmetry.set(spin, crystal_structure, num_symmetry_operations, rotation, translation,
                 read_xml_element(pt, "qes:espresso.input.symmetry_flags.no_t_rev", ost),
                 read_xml_element(pt, "qes:espresso.input.symmetry_flags.noinv", ost));
    
    // read nbnd
    std::vector<int> nbnd(spin.num_independent_spins());
    if (spin.num_independent_spins()==2)
    {
        nbnd[0] = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.band_structure.nbnd_up", ost));
        nbnd[1] = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.band_structure.nbnd_dw", ost));
    }
    else
    {
        nbnd[0] = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.band_structure.nbnd", ost));
    }
    
    // read nks (no. of irreducible k-points)
    const int num_irreducible_kpoints = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.band_structure.nks", ost));
    ...
    BOOST_FOREACH (auto child, pt.get_child("qes:espresso.output.band_structure")) 
    {
        if (child.first=="ks_energies")
        {
            *ost << "  Irreducible k-point: " << ik << std::endl;
            ...
            string_to_dvect(read_xml_element_child(child.second, "qes:espresso.output.band_structure.ks_energies",
            ...
            kweight[ik] = boost::lexical_cast<double>(read_xml_element_child(child.second, "qes:espresso.output.band_structure.ks_energies",
                                                                                "k_point.<xmlattr>.weight", ost));            
            num_G_at_k[ik] = boost::lexical_cast<int>(read_xml_element_child(child.second, "qes:espresso.output.band_structure.ks_energies",
                                                                                "npw", ost));

            int nbands_tmp = boost::lexical_cast<double>(read_xml_element_child(child.second, "qes::espresso.output.band_structure_ks_energies",
    ...
    bloch_states.set_qe_xml(nbnd, eigenvalues, method.calc_mode(),
                            boost::lexical_cast<double>(read_xml_element(pt, "qes:espresso.output.band_structure.nelec", ost)));
    ...
    // set FFT grid
    const int nr1 = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.basis_set.fft_grid.<xmlattr>.nr1", ost));
    const int nr2 = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.basis_set.fft_grid.<xmlattr>.nr2", ost));
    const int nr3 = boost::lexical_cast<int>(read_xml_element(pt, "qes:espresso.output.basis_set.fft_grid.<xmlattr>.nr3", ost));
    plane_wave_basis.setup_FFT(nr1, nr2, nr3);
    
    // Ewald energy
    total_energy.set_ewald_energy(potentials.is_heg(),
                                  boost::lexical_cast<double>
                                  (read_xml_element(pt, "qes:espresso.output.total_energy.ewald", ost)));

    // gamma_only
    if (read_xml_element(pt, "qes:espresso.output.basis_set.gamma_only", ost) == "true")
    {
        plane_wave_basis.set_gamma_only(true); // else: gamma_only = false (default)
    }
  
    // Pseudopotential information
    if (read_xml_element(pt, "qes:espresso.output.algorithmic_info.uspp", ost) == "true" ||
        read_xml_element(pt, "qes:espresso.output.algorithmic_info.paw", ost) == "true")
    {
