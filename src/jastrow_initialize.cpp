...
// An appropriate factor is later multiplied in normalize_A_long() to make it in Hartree unit.
// F_long_ is also initialized there.
void Jastrow::set_A_long_unnormalized(const std::vector<std::vector<double> > &A_long)
{
...
{
    set_A_long_unnormalized(A_long);
}
...
void Jastrow::normalize_A_long(const CrystalStructure &crystal_structure,
