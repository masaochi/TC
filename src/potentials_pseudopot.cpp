...
void Potentials::set_Vpp_local(const std::vector<double> &Z_valence,
                               ...
                               PlaneWaveBasis &plane_wave_basis)
{
    ...
    for (int iatomic_species=0; iatomic_species<num_atomic_species; iatomic_species++)
    {
        Vpp_local_atom_G[iatomic_species].resize(npw);
    }
    const double param_ewald = PI/std::cbrt(crystal_structure.unit_cell_volume()); // cbrt = cubic root
    ...
    for (int iatomic_species=0; iatomic_species<num_atomic_species; iatomic_species++)
    {
        ...
        for (int imesh=0; imesh<mesh_size; imesh++) // radial mesh in pseudopot.
        {
            ...
            {
                local_temp[imesh]
                    = pseudo_local[iatomic_species][imesh]
        ...
        for (int ipw=0; ipw<npw; ipw++)
        {
            ...
            Vpp_local_atom_G[iatomic_species](ipw) = simpson(local_temp2, iatomic_species);
            ...
            Vpp_local_atom_G[iatomic_species](ipw) *= (FourPI/unit_cell_volume);
        } // ipw
    } // iatomic_species
...
void Potentials::set_upf(const std::vector<double> &Z_valence,
                         const std::vector<std::vector<double> > &pseudo_rmesh,
...
{   
    ...
    // set Vpp_local_
    set_Vpp_local(Z_valence, pseudo_local, crystal_structure, plane_wave_basis);
}
...
//                * boost::math::sph_bessel(lbeta, kG*rmesh);
        ...
        // calculate i^l Y_lm(e_{k+G})^* beta_jl(|k+G|)
        return_vector[lbeta](ipw_at_k) = cfactor * ylm[lbeta][0](ipw_at_k) * beta_jl; // im=0 
        for (int im=1; im<lbeta+1; im++)
        {
            return_vector[lbeta - im](ipw_at_k) = cfactor * std::conj(ylm[lbeta][im](ipw_at_k)) * beta_jl;
            return_vector[lbeta + im](ipw_at_k) = cfactor * ylm[lbeta][im](ipw_at_k) * beta_jl;
        }
    }
